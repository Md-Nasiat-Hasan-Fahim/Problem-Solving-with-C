#include<bits/stdc++.h>
using namespace std;

#define FastIO ios_base::sync_with_stdio(false), cin.tie(0)
#define endl '\n'
#define int long long
#define all(x) x.begin(),x.end()
#define pb push_back
#define YES cout<<"YES\n"
#define NO cout<<"NO\n"
#define check(x) cout << (#x) << " is " << (x) << endl;
const int mod = 1e9+7;

struct tr{
  vector<int>tree;
  void init(int n)
  {
    tree.assign(4*n,0);
  }
  int update(int idx, int s, int e, int node)
  {
    if(idx<s||idx>e) return tree[node];
    if(idx==s&&idx==e)
    {
      tree[node]=!tree[node];
      return tree[node];
    }
    int mid = (s+e)/2;
    tree[node] = update(idx,s,mid,2*node) + update(idx,mid+1,e,2*node+1);
    return tree[node];
  }
  int sum(int i, int j, int s, int e, int node)
  {
    if(i>e||j<s) return 0;
    if(s>=i&&e<=j) return tree[node];
    int mid = (s+e)/2;
    return sum(i,j,s,mid,2*node) + sum(i,j,mid+1,e,2*node+1);
  }
  bool abc(int x, int n, int val)
  {
    // cout<<x<<" "<<val<<" "<<sum(x+1,n,1,n,1)<<endl;
    return sum(x+1,n,1,n,1)<=val;
  }
};

signed main()
{
    //freopen("","r+",stdin);
    //freopen("","w+",stdout);
    FastIO;
    int Master0fTheSEA=1;
    // cin>>Master0fTheSEA;
    for(int tttttttt=1;tttttttt<=Master0fTheSEA;tttttttt++)
    {
      int n;
      cin>>n;
      tr tree;
      vector<int>vec(n);
      tree.init(n+3);
      for(int i=1;i<=n;i++) tree.update(i,1,n,1);
      // cout<<tree.sum(5,5,1,5,1)<<endl;
      // cout<<tree.abc(4,n,0)<<endl;
      for(int i=0;i<n;i++) cin>>vec[i];
      stack<int> st;
      for(int i=n-1;i>-1;i--)
      {
        int hi = n, lo = 0;
        while(hi-lo>1)
        {
          int mid = (hi+lo)/2;
          if(tree.abc(mid,n,vec[i])) hi = mid;
          else lo = mid;
        }
        st.push(hi);
        tree.update(hi,1,n,1);
        // cout<<hi<<endl;
      }
      while(!st.empty())
      {
        cout<<st.top()<<" ";
        st.pop();
      }
      cout<<endl;
    }
    //cerr << "Time : " << 1000 * ((double)clock()) / (double)CLOCKS_PER_SEC <<   "ms\n";
    return 0;
}
